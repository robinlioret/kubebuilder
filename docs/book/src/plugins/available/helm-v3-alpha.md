# Helm Plugin `(helm/v3-alpha)`

The Helm plugin **v3-alpha** provides a way to package your project as a Helm chart, enabling distribution in Helm’s native format.
...
The **helm/v3-alpha** plugin converts the bundle (`dist/install.yaml`) into a Helm chart that mirrors your project.
...
Use the **helm/v3-alpha** plugin if:
...
kubebuilder edit --plugins=helm/v3-alpha --force
```
...
kubebuilder edit --plugins=helm/v3-alpha --manifests=manifests/custom-install.yaml

# Write chart to a custom output directory
kubebuilder edit --plugins=helm/v3-alpha --output-dir=charts
